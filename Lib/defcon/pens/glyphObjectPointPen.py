...
class GlyphObjectPointPen(AbstractPointPen):
    ...
    def beginPath(self):
        self._contour = self._glyph.contourClass(self._glyph._dispatcher)

    def endPath(self):
        if len(self._contour) == 1:
            point = self._contour[0]
            anchor = self._glyph.anchorClass(self._glyph._dispatcher)
            anchor.x = point.x
            anchor.y = point.y
    ...
    def addComponent(self, baseGlyphName, transformation):
        component = self._glyph.componentClass(self._glyph._dispatcher)
